...
import com.pj.alfresco.Repositories.PieceJointeAutorisationRepo;
...
@RestController
@RequestMapping({ "/PieceJointeAutorisation" })
public class PieceJointeAutorisationController {
    @Autowired
    PieceJointeAutorisationRepo pieceJointeAutorisationRepo;
    ...
    public List<PieceJointeAutorisation> AllSeverite(@PathVariable long idAo) {
         ...
         */
        return pieceJointeAutorisationRepo.findByIdAutorisationOrderByIdDesc(idAo);
    }

    @GetMapping({ "/allFiles/{idAo}/{sModule}" })
    public List<PieceJointeAutorisation> allDocsByIdAndSousModule(@PathVariable long idAo, @PathVariable String sModule) {
        Session session = conf();
        return pieceJointeAutorisationRepo.findByIdAutorisationAndSousModuleOrderByIdDesc(idAo, sModule);
    }
    ...
    public void rec(@PathVariable String idAlfresco, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        response.addHeader("Content-Disposition",
                "attachment; filename=" + URLEncoder.encode(pieceJointeAutorisationRepo.findByIdAlfresco(id_alfresco).getName(), "UTF-8"));
//		response.addHeader("Content-Disposition",
//				"attachment; filename=" + convnertionMarcheRepo.findByIdAlfresco(id_alfresco).getName());
    ...
    public long processFile(@RequestParam("file") MultipartFile[] files, @RequestParam("id") long id,
        ...
        for (MultipartFile file : files) {
            ...
            PieceJointeAutorisation pj = new PieceJointeAutorisation();
            ...
            PieceJointeAutorisation pjUpdate = (PieceJointeAutorisation) this.pieceJointeAutorisationRepo.save(pj);
            ...
            for (CmisObject child : root.getChildren()) {
                if (child.getName().equals("Ao")) {
                    ...
                    pjUpdate.setIdAlfresco(newDoc.getId());
                    this.pieceJointeAutorisationRepo.save(pjUpdate);
