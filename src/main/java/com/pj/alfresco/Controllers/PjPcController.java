...
import com.pj.alfresco.Repositories.PjPcRepo;
...
public class PjPcController {
	...
	public List<PjPc> AllSeverite(@PathVariable long idPc) {
		Session session = this.conf();
		return this.pjPcRepo.findByIdPcOrderByIdDesc(idPc);
	}

	@GetMapping({"/allFiles/{idPc}/{sModule}"})
	public List<PjPc> allDocsByIdAndSousModule(@PathVariable long idPc, @PathVariable String sModule) {
		Session session = this.conf();
		return this.pjPcRepo.findByIdPcAndSousModuleOrderByIdDesc(idPc, sModule);
	}
	...
	public void rec(@PathVariable String idAlfresco, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		response.addHeader("Content-Disposition",
				"attachment; filename=" + URLEncoder.encode(this.pjPcRepo.findByIdAlfresco(id_alfresco).getName(), "UTF-8"));
//        response.addHeader("Content-Disposition", "attachment; filename=" + this.convnertionMarcheRepo.findByIdAlfresco(id_alfresco).getName());
		System.out.println("aa"+URLEncoder.encode(this.pjPcRepo.findByIdAlfresco(id_alfresco).getName(), "UTF-8"));
	...
	public long processFile(@RequestParam("file") MultipartFile[] files, @RequestParam("id") long id, @RequestParam("sModule") String sModule) throws IOException {
		...
		for(int var9 = 0; var9 < var8; ++var9) {
			...
			PjPc pjUpdate = (PjPc) this.pjPcRepo.save(pj);
